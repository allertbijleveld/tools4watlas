testthat::test_that("atl_as_sf handles valid data and columns", {
  # Create test data
  data <- data.table::data.table(x = c(1, 2, 3, 4), 
                                 y = c(5, 6, 7, 8), 
                                 value = c(9, 10, 11, 12), 
                                 category = c("A", "B", "C", "D"))
  
  # Convert to sf with x and y columns
  d_sf <- tools4watlas::atl_as_sf(data, "x", "y", 
  ...
  testthat::expect_equal(ncol(d_sf), 3)  # geometry, value, category
...
testthat::test_that("atl_as_sf handles missing columns for x and y", {
  ...
  data <- data.table::data.table(lon = c(1, 2, 3, 4), 
                                 lat = c(5, 6, 7, 8))
  
  # Expect error when the 'x' and 'y' columns are not present in the data
  testthat::expect_error(tools4watlas::atl_as_sf(data, "x", "y"), 
...
testthat::test_that("atl_as_sf excludes rows with NA values in x or y", {
  ...
  data <- data.table::data.table(x = c(1, 2, NA, 4), 
                                 y = c(5, 6, 7, NA), 
                                 value = c(9, 10, 11, 12))
  
  # Convert to sf and check that rows with NA in x or y are excluded
  d_sf <- tools4watlas::atl_as_sf(data, "x", "y", additional_cols = "value")
...
testthat::test_that("atl_as_sf handles missing additional columns gracefully", {
  ...
  testthat::expect_equal(names(d_sf), c("geometry"))
})
