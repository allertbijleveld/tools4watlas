...
#' atl_plot_tag_osm(data = d_sf, tag = NULL, mapID = map, 
...
atl_plot_tag_osm <- function(data,    
  ...
  if (color_by == "nbs") {
    ...
    color_by_values <- log10(apply(cbind(
      data$VARX, data$VARY), 1, function(x) max(x)))
  ...
  rbPal <- colorRampPalette(c('white', 'lightyellow', 'yellow', 'orange', 
  ...
  raster::scalebar(
    scalebar * 1000, xy_scale, type = 'line', divs = 4, lwd = 3, 
  ...
  legend(
    legend, legend = legend_cuts, col = legend_cuts_col, pch = 15, bty = "n",
