...
atl_plot_tag_osm <- function(data, 
		...
		assertthat::assert_that(!is.data.frame(data),
			msg = "Data is provided as a data.frame. First use atl_make_spatial() 
			to transform to a spatial object with the osm() 
	...
	if(is.null(tag)){tag<-data$tag[1]}else{
		...
		if(color_by=="SD")	{ 
					color_by_values <- log10(apply(cbind(data$VARX, data$VARY), 1, 
					                               function(x) max(x)))
					color_by_title<-paste("log10(max(VARX,VARY))", '\n', "tag ",
					                      tag,sep="")
					}
		if(color_by=="time")	{ 
					color_by_values <-as.numeric(difftime(data$time, min(data$time)), 
					                             unit="hours")
		...
		### if fullname is NULL then plot in graphics device  
			if(is.null(fullname)){
					...
					png(filename = fullname, width = px_width, height = px_height, 
					    units = "px")
					}
	...
	## specify plot 
		...
		# add title 
			mtext(paste("from ", min(data$time)," UTC\nto ", max(data$time),
			            " UTC",sep=""), line = 1.7, cex=1, col="white")
		# add towers
			if(!is.null(towers)){points(towers$X,towers$Y,pch=23, cex=2,col=2,bg=1)}
		## make color scale
			rbPal <- colorRampPalette(c('white', 'light yellow', 'yellow', 'orange',
		...
		## add legend 
			...
			legend(Legend, legend=legend_cuts, col =legend_cuts_col, pch=15, bty="n", 
