...
#' @param fpath The full path (including path and file name) of the csv file. If left empty, the example data file supplied with \code{tools4watlas} will be used.
...
#' @export
atl_get_data_csv<-
function (fpath=system.file("extdata", "redknot_2707_WATLAS_exampledata.csv", package="tools4watlas")){		
	...
	## check that the input has all the typical WATLAS-columns 
		names_expected <- c("posID", "TAG", "tag","TIME","time","X","Y","NBS","VARX","VARY","COVXY")
		names_found<-names_expected %in% names(data)
		print(stringr::str_glue("WARNING: Expected column {names_expected[!names_found]} not found in csv file"))
