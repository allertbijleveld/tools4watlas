...
atl_add_tidaldata <- function(data,
  ...
  # global variables
  x <- y <- x_raw <- y_raw <- row_id <- low_time <- . <- complete.cases <- NULL
  ...
  assertthat::assert_that(
    nrow(data) > 0, msg = "Input doesn't have any rows"
  ) 
  ...
  temp_data <- data.table::merge.data.table(
    data, high_tide_data, by.x = "datetime", by.y = "high_start_time", 
    all = TRUE)
  ...
  temp_data[, tidaltime := as.numeric(difftime(
    datetime, datetime[1], units = "mins")), by = tideID]
  temp_data <- temp_data[complete.cases(temp_data)]
  
  # add time2lowtide
  temp_data[tide_data, 
            time2lowtide := as.numeric(difftime(time, low_time, 
                                                units = "mins")), 
            on = "tideID"]
  ...
  temp_data[, temp_time := lubridate::round_date(datetime, 
