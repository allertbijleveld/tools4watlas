...
atl_plot_tag <- function(data,
  ...
  if(is.null(fullname)){
    ...
    png(filename = fullname, height = h, width = w, 
        units = "in", res = 96)
  }
  ...
  # plot map
  sp::plot(land, xlab = "x (km)", ylab = "y (km)", asp = 1, xaxt = "n", 
           yaxt = "n", ylim = yrange, xlim = xrange, xaxs = "i", 
           cex.lab = 1.5)	
  axis(1, at = prettyX, labels = prettyXkm)
  axis(2, at = prettyY, labels = prettyYkm)
  sp::plot(land, add = TRUE, 
           col = COL_LAND[as.numeric(as.factor(land@data$SOORT))])		
