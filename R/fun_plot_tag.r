...
#' atl_plot_tag(data = d_sf, tag = NULL, fullname = NULL, buffer = 1,
#'              color_by = "time")
#' 
#' atl_plot_tag(data = d_sf, tag = NULL, fullname = NULL, buffer = 1,
#'              color_by = "sd")
...
atl_plot_tag <- function(data,
  ...
  # Plot elements
  plot(sf::st_geometry(land_data), xlab = "x (km)", ylab = "y (km)", asp = 1,
  ...
  plot(sf::st_geometry(mudflats_data), add = TRUE, col = COL_MUD, 
       border = COL_MUD)
  ...
  plot(sf::st_geometry(lakes_data), add = TRUE, col = COL_WATER, 
       border = COL_WATER)
  
  # Title
  mtext(paste("Tag ", tag, "\nFrom ", min(data$datetime), " UTC\nTo ", 
              max(data$datetime), " UTC", sep = ""), 
  ...
  # Add scalebar
  fr <- 0.02
  xy_scale <- c(par("usr")[1] + diff(par("usr")[1:2]) * fr, 
  ...
  legend(legend, legend = legend_cuts, col = legend_colors, pch = 15, 
