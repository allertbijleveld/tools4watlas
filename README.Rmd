...
**Vignettes**:
  ...
  How to process data (calculate speed & angles, filter, smooth and add tidal data)
...
When contributing, please follow the existing structure of the repository and [style](http://adv-r.had.co.nz/Style.html). You can use [`styler`](https://styler.r-lib.org/) to adapt your code and [`lintr`](https://lintr.r-lib.org/) to check the style. Note that not all code follows this style at the moment, but new code should aim to be consistent with it. 

Before submitting a pull request to the original “upstream” repository, please make sure the code passes all R CMD checks (`devtools::check()`). 
