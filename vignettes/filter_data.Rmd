...
A bird's speed can be calculated from sequential positions and used for filtering unrealisticly high speeds. This is straightforward for positions with very high speeds between itself and the positions before and after (the so-called incoming and outgoing speed, respectively). See **[Gupte et al. 2022](https://doi.org/10.1111/1365-2656.13610)** for more background.

To choose a maximum speed threshold, it is important to plot the distribution of speed. We found that its better to have a large maximum speed threshold and only filter out extreme speeds. Remaining erroneous positions can be filter out by smoothing the data (e.g. by [median smoothing](https://allertbijleveld.github.io/tools4watlas/articles/smooth_and_thin_data.html#median-smooth-data)).
