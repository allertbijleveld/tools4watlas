...
# plot
bm +
  geom_point(data = data, aes(x, y), alpha = 0.1, color = "darkolivegreen4")
```
...
Provides a range of different basemap options. For this we first have to transform the data to WGS 84 to extract the basemap with the bounding box and then transform them to a Mercator projection to plot the data on top of the map.

Unfortunatly, sometimes the type = `"bing"` does not work.
...
# get bounding box
bbox <- atl_bbox(d_sf, asp = "16:9", buffer = 1000)
