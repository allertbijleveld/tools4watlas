...
tags <- all_tags[season == 2023 &
                   species %in% c("red knot", "redshank")]$tag
...
data_split[[1]][1:5, ] |> knitr::kable(digits = 2)
```
...
```{r, fig.cap = "Overview plot by tag", fig.width = 7, fig.height = 4}
# Plot data
ggplot(data) +
  geom_path(aes(x, y, color = tag), alpha = 0.1) +
  geom_point(aes(x, y, color = tag), size = 0.5) +
  coord_sf(crs = 32631) + 
  labs(x = "", y = "") +
  theme_bw()
```
...
# Plot heatmap
ggplot(data_heatmap) +
  geom_tile(aes(x_round, y_round, fill = N), linewidth = 0.1, show.legend = T) +
  scale_fill_viridis(
    option = "A", discrete = FALSE, trans = "log10", name = "N positions",
    breaks = trans_breaks("log10", function(x) 10^x), 
    labels = trans_format("log10", math_format(10^.x)), 
