...
```{r, out.width="50%", fig.cap = "Distribution of flight speeds"}	
library(ggplot2)
...
```{r, out.width="100%", dpi=300, fig.cap = "Smoothed track (black) on top of raw track (red)"}	
# select data from first list
d1_raw <- data_split[[1]]
d1_smooth <- data_smooth[[1]]

# subset first day
from <- min(d1_raw[, datetime]) + 6 * 3600
to <- min(d1_raw[, datetime]) + 7 * 3600
...
# Plot 
ggplot() +
  geom_path(data = d1_raw, aes(x, y), color = "firebrick3", linewidth = 0.8) +
  geom_path(data = d1_smooth, aes(x, y), color = "black", linewidth = 0.8) +
  coord_sf(crs = 32631) + 
  labs(x = "", y = "") +
  theme_void()
...
head(data[, .(posID, tag, datetime, tideID, tidaltime, time2lowtide, waterlevel)]) |>
